...
- In generale, il metodo codegen() chiamerà ricorsivamente il metodo codegen() dei blocchi sottostanti per generare il loro codice. Inoltre:
	- il metodo ritorna l'ultimo valore ritornato dal blocco
	- il metodo deve tenere traccia dell'indice incrementale delle variabili senza nome (<b>TO BE DEFINED</b>)
...
| ImperPartAST     | IOActionAST ioAction                                     | createMain("main", void)<br />openFunction()<br />ioAction.codegen()<br />mountCode(code, ioAction.code)<br />closeFunction() |
...
| DeclTypeAST      |                                                          |                                                              |
| DefValueAST      | String id, ExprAST expr                                  | if (global) <br />  createFunctionDefinition(id, expr.type)<br /><br />  lformarg.codegen()<br />  mountCode(code, lformarg)<br />  openFunction()<br />  exprIndex = expr.codegen()<br />  mountCode(code, expr.code)<br />  createReturn(exprIndex, expr.type)<br />  closeFunction()<br />else <br />  exprIndex = expr.codegen()<br />  mountCode(code, expr.code)<br />  idIndex = createAlloca(id, expr.type, expr.subexpressions.size)<br />  createStore(expr.type, idIndex, exprIndex) |
